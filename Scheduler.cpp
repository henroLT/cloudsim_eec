...
//  CloudSim
//
//  Strategy summary (mirrors the Snooze paper):
//
//  Placement   – First-fit within the active machines that already hold a
//                compatible VM, falling back to any machine with enough free
//                memory for a brand-new VM.
//  Overload    – Handled reactively via MemoryWarning: migrate the heaviest VM
//                out to the machine with the most headroom.
//  Underload   – Detected in PeriodicCheck (memory util < 20 %).  All-or-nothing
//                migration of every VM on the underloaded machine to more loaded
//                machines; the emptied machine is then powered to S3.
//  Consolidation – Sercon-style greedy: sort machines by ascending utilisation,
//                  take the least-loaded one, plan destinations for every VM
//                  (all-or-nothing), execute the plan, then let
//                  shutdownIdleMachines power the now-empty machine to S3.
//  Power mgmt  – Machine is only powered down when it is truly empty (no VMs,
//                no active tasks) AND at least one other machine of the same
//                CPU type stays active as a hot spare.
//  SLA warning – Boost every core on the offending machine to P0 immediately.
//
...
void Scheduler::powerDownMachine(MachineId_t machine) {
    ...
    if (info.active_tasks > 0 || info.active_vms > 0) return;
